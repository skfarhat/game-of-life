...
public class LifeCellTest {
    ...
    public void testRemoveDeadAgentsFromCell() throws AgentAlreadyDeadException, GridCreationException, InvalidPositionException, SurfaceAlreadyPresent {
        ...
        final int nAgents = 10 + Utils.randomPositiveInteger(91); // number of core.agents to create
        final int nKill = 1 + Utils.randomPositiveInteger(nAgents-1); // number of core.agents to kill
        ArrayList<LifeAgent> allAgents = new ArrayList<>(nAgents); // list of created core.agents
        ArrayList<LifeAgent> agentsKilled = new ArrayList<>(nKill); // list of killed core.agents
        ArrayList<LifeAgent> agentsAlive = new ArrayList<>(nAgents - nKill); // list of all core.agents that are still alive
        ...
        assertEquals("size of the returned list should be equal to the number of killed core.agents",
                nKill, returnedList.size());
        // all core.agents in returnedList should be present in the agentsKilled List
        for (LifeAgent agent: returnedList) {
            assertFalse(agent.isAlive()); // check the core.agents is dead
